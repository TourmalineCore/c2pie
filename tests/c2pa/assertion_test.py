...
def test_ingredient_assertion_with_manifest_ref_schema():
    expected_c2pa_manifest = {"url": "self#jumbf=c2pa/urn:uuid:abc", "alg": "sha256", "hash": b"\xde\xad"}
    test_assertion = IngredientAssertion(
        title="original.jpg", dc_format="image/jpeg", c2pa_manifest_ref=expected_c2pa_manifest
    )
    assert test_assertion.schema["c2pa_manifest"] == expected_c2pa_manifest
